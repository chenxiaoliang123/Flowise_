...
const DocumentStoreDetails = () => {
    ...
    const openPreviewSettings = (id) => {
        navigate('/cxl/document-stores/' + storeId + '/' + id)
    }

    const showStoredChunks = (id) => {
        navigate('/cxl/document-stores/chunks/' + storeId + '/' + id)
    }

    const onDocLoaderSelected = (docLoaderComponentName) => {
        setShowDocumentLoaderListDialog(false)
        navigate('/cxl/document-stores/' + storeId + '/' + docLoaderComponentName)
    }
    ...
    const onLoaderDelete = async (file) => {
        const confirmPayload = {
            title: `删除`,
            description: `删除加载器 ${file.loaderName} ? 这将删除所有关联的文档块。`,
    ...
    const onStoreDelete = async () => {
        const confirmPayload = {
            title: `删除`,
            description: `删除知识库 ${getSpecificDocumentStore.data?.name} ? 这将删除所有相关的加载器和文档块。`,
            confirmButtonName: 'Delete',
            cancelButtonName: '关闭'
        }
        const isConfirmed = await confirm(confirmPayload)
                ...
                {error ? (
                    ...
                    <Stack flexDirection='column' sx={{ gap: 3 }}>
                        <ViewHeader
                            ...
                            description={documentStore?.description}
                            onBack={() => navigate('/cxl/document-stores')}
                            onEdit={() => onEditClicked()}
                        >
                            <IconButton onClick={onStoreDelete} size='small' color='error' title='删除知识库' sx={{ mr: 2 }}>
                                <IconTrash />
                            </IconButton>
                                ...
                                <Table sx={{ minWidth: 650 }} aria-label='simple table'>
                                    ...
                                    >
                                        <TableRow>
                                            ...
                                            <StyledTableCell>文档加载器</StyledTableCell>
                                            <StyledTableCell>拆分器</StyledTableCell>
                                            <StyledTableCell>原始文件(s)</StyledTableCell>
                                            <StyledTableCell>文件块</StyledTableCell>
                                            <StyledTableCell>字符</StyledTableCell>
