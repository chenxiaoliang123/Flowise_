    ...
    const onPreviewChunks = async () => {
        if (checkMandatoryFields()) {
            ...
            try {
                ...
                setLoading(false)
                enqueueSnackbar({
                    message: `未有预览块：${typeof error.response.data === 'object' ? error.response.data.message : error.response.data}`,
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'error',
                        action: (key) => (
        ...
        if (checkMandatoryFields()) {
            ...
            try {
                ...
                if (processResp.data) {
                    ...
                    navigate('/cxl/document-stores/' + storeId)
                }
            } catch (error) {
                setLoading(false)
                enqueueSnackbar({
                    message: `处理分块失败：${typeof error.response.data === 'object' ? error.response.data.message : error.response.data}`,
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'error',
                        action: (key) => (
